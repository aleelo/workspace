...
class Appointments_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        if ($id) {
            $where .= " AND $appointments_table.id=$id";
        }
        ...
        if ($leads_only) {
            ...
            $where .= " AND $appointments_table.is_lead=1";
        }

        $status = $this->_get_clean_value($options, "status");
        if ($status) {
            $where .= " AND $appointments_table.lead_status_id='$status'";
        }

        $source = $this->_get_clean_value($options, "source");
        if ($source) {
            $where .= " AND $appointments_table.lead_source_id='$source'";
        }

        $owner_id = $this->_get_clean_value($options, "owner_id");
        if ($owner_id) {
            $where .= " AND $appointments_table.owner_id=$owner_id";
        }
        ...
        if ($show_own_clients_only_user_id) {
            $where .= " AND ($appointments_table.created_by=$show_own_clients_only_user_id OR $appointments_table.owner_id=$show_own_clients_only_user_id)";
        }


        $quick_filter = $this->_get_clean_value($options, "quick_filter");
        if ($quick_filter) {
            $where .= $this->make_quick_filter_query($quick_filter, $appointments_table, $projects_table, $invoices_table, $invoice_payments_table, $estimates_table, $estimate_requests_table, $tickets_table, $orders_table, $proposals_table);
        }

        $start_date = $this->_get_clean_value($options, "start_date");
        if ($start_date) {
            $where .= " AND DATE($appointments_table.created_date)>='$start_date'";
        }
        $end_date = $this->_get_clean_value($options, "end_date");
        if ($end_date) {
            $where .= " AND DATE($appointments_table.created_date)<='$end_date'";
        }

        $label_id = $this->_get_clean_value($options, "label_id");
        if ($label_id) {
            $where .= " AND (FIND_IN_SET('$label_id', $appointments_table.labels)) ";
        }

        $select_labels_data_query = $this->get_labels_data_query();

        $client_groups = $this->_get_clean_value($options, "client_groups");
        $where .= $this->prepare_allowed_client_groups_query($appointments_table, $client_groups);

        //prepare custom fild binding query
        $custom_fields = get_array_value($options, "custom_fields");
        $custom_field_filter = get_array_value($options, "custom_field_filter");
        $custom_field_query_info = $this->prepare_custom_field_query_string($custom_field_type, $custom_fields, $appointments_table, $custom_field_filter);
        ...
        if ($search_by) {
            ...
            if ($leads_only) {
                ...
                $where .= $this->get_custom_field_search_query($appointments_table, "leads", $search_by);
            } else {
                $where .= $this->get_custom_field_search_query($appointments_table, "clients", $search_by);
            }
