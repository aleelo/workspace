...
class Items_list_model extends Crud_model {

    protected $table = null;

    function __construct() {
        $this->table = 'items_list';
    ...
    function get_details($options = array()) {
        ...
        if ($id) {
            $where .= " AND $items_list_table.id=$id";
        }
        ...
        if ($leads_only) {
            ...
            $where .= " AND $items_list_table.is_lead=1";
        }

        $status = $this->_get_clean_value($options, "status");
        if ($status) {
            $where .= " AND $items_list_table.lead_status_id='$status'";
        }

        $source = $this->_get_clean_value($options, "source");
        if ($source) {
            $where .= " AND $items_list_table.lead_source_id='$source'";
        }

        $owner_id = $this->_get_clean_value($options, "owner_id");
        if ($owner_id) {
            $where .= " AND $items_list_table.owner_id=$owner_id";
        }

        $created_by = $this->_get_clean_value($options, "created_by");
        if ($created_by) {
            $where .= " AND $items_list_table.created_by=$created_by";
        }
        ...
        if ($show_own_clients_only_user_id) {
            $where .= " AND $items_list_table.dep_head_id=$show_own_clients_only_user_id";
        }


        $quick_filter = $this->_get_clean_value($options, "quick_filter");
        if ($quick_filter) {
            $where .= $this->make_quick_filter_query($quick_filter, $items_list_table, $projects_table, $invoices_table, $invoice_payments_table, $estimates_table, $estimate_requests_table, $tickets_table, $orders_table, $proposals_table);
        }

        $start_date = $this->_get_clean_value($options, "start_date");
        if ($start_date) {
            $where .= " AND DATE($items_list_table.created_date)>='$start_date'";
        }
        $end_date = $this->_get_clean_value($options, "end_date");
        if ($end_date) {
            $where .= " AND DATE($items_list_table.created_date)<='$end_date'";
        }

        $label_id = $this->_get_clean_value($options, "label_id");
        if ($label_id) {
            $where .= " AND (FIND_IN_SET('$label_id', $items_list_table.labels)) ";
        }

        $select_labels_data_query = $this->get_labels_data_query();

        $client_groups = $this->_get_clean_value($options, "client_groups");
        $where .= $this->prepare_allowed_client_groups_query($items_list_table, $client_groups);

        //prepare custom fild binding query
        $custom_fields = get_array_value($options, "custom_fields");
        $custom_field_filter = get_array_value($options, "custom_field_filter");
        $custom_field_query_info = $this->prepare_custom_field_query_string($custom_field_type, $custom_fields, $items_list_table, $custom_field_filter);
        ...
        $available_order_by_list = array(
            "id" => $items_list_table . ".id",
            "nameSo" => $items_list_table . ".nameSo",
            "short_name_SO" => $items_list_table . ".short_name_SO",
            "nameEn" => $items_list_table . ".nameEn",
            "short_name_EN" => $items_list_table . ".short_name_EN",
            "email" => $items_list_table . ".email",
            "DeptHead" => "CONCAT($users_table.first_name, ' ', $users_table.last_name)",
            "secretary" => "CONCAT(sec.first_name,' ',sec.last_name)",
            "remarks" => $items_list_table . ".remarks",
        ...
        if ($search_by) {
            ...
            $where .= " AND (";
            $where .= " $items_list_table.id LIKE '%$search_by%' ESCAPE '!' ";
            $where .= " OR $items_list_table.nameSo LIKE '%$search_by%' ESCAPE '!' ";
            $where .= " OR $items_list_table.short_name_SO LIKE '%$search_by%' ESCAPE '!' ";
            $where .= " OR $items_list_table.nameEn LIKE '%$search_by%' ESCAPE '!' ";
            $where .= " OR $items_list_table.short_name_EN LIKE '%$search_by%' ESCAPE '!' ";
            $where .= " OR $items_list_table.email LIKE '%$search_by%' ESCAPE '!' ";
            $where .= " OR $items_list_table.nameSo LIKE '%$search_by%' ESCAPE '!' ";
            ...
            $where .= " OR $items_list_table.remarks LIKE '%$search_by%' ESCAPE '!' ";
            $where .= " OR (SELECT GROUP_CONCAT($labels_table.title, ', ') FROM $labels_table WHERE FIND_IN_SET($labels_table.id, $items_list_table.labels)) LIKE '%$search_by%' ESCAPE '!' ";

            if ($leads_only) {
                $where .= " OR $lead_status_table.title LIKE '%$search_by%' ESCAPE '!' ";
                $where .= $this->get_custom_field_search_query($items_list_table, "leads", $search_by);
            } else {
                $where .= $this->get_custom_field_search_query($items_list_table, "clients", $search_by);
            }
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS $items_list_table.*
        FROM $items_list_table
        -- LEFT JOIN $users_table ON $users_table.id = $items_list_table.dep_head_id
        -- LEFT JOIN $users_table as sec ON sec.id = $items_list_table.secretary_id
        $join_custom_fieds               
        WHERE $items_list_table.deleted=0 $where $custom_fields_where  
        $order $limit_offset";
