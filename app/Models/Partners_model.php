...
class Partners_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        if ($id) {
            $where .= " AND $partners_table.id=$id";
        }
        ...
        if ($leads_only) {
            ...
            $where .= " AND $partners_table.is_lead=1";
        }

        $status = $this->_get_clean_value($options, "status");
        if ($status) {
            $where .= " AND $partners_table.lead_status_id='$status'";
        }

        $source = $this->_get_clean_value($options, "source");
        if ($source) {
            $where .= " AND $partners_table.lead_source_id='$source'";
        }

        $owner_id = $this->_get_clean_value($options, "owner_id");
        if ($owner_id) {
            $where .= " AND $partners_table.owner_id=$owner_id";
        }

        $created_by = $this->_get_clean_value($options, "created_by");
        if ($created_by) {
            $where .= " AND $partners_table.created_by=$created_by";
        }
        ...
        if ($show_own_clients_only_user_id) {
            $where .= " AND ($partners_table.created_by=$show_own_clients_only_user_id OR $partners_table.owner_id=$show_own_clients_only_user_id)";
        }
        ...
        if ($quick_filter) {
            $where .= $this->make_quick_filter_query($quick_filter, $partners_table, $projects_table, $invoices_table, $invoice_payments_table, $estimates_table, $estimate_requests_table, $tickets_table, $orders_table, $proposals_table);
        }

        $start_date = $this->_get_clean_value($options, "start_date");
        if ($start_date) {
            $where .= " AND DATE($partners_table.created_date)>='$start_date'";
        }
        $end_date = $this->_get_clean_value($options, "end_date");
        if ($end_date) {
            $where .= " AND DATE($partners_table.created_date)<='$end_date'";
        }

        $label_id = $this->_get_clean_value($options, "label_id");
        if ($label_id) {
            $where .= " AND (FIND_IN_SET('$label_id', $partners_table.labels)) ";
        }

        $select_labels_data_query = $this->get_labels_data_query();

        $client_groups = $this->_get_clean_value($options, "client_groups");
        $where .= $this->prepare_allowed_client_groups_query($partners_table, $client_groups);

        //prepare custom fild binding query
        $custom_fields = get_array_value($options, "custom_fields");
        $custom_field_filter = get_array_value($options, "custom_field_filter");
        $custom_field_query_info = $this->prepare_custom_field_query_string($custom_field_type, $custom_fields, $partners_table, $custom_field_filter);
        ...
        $available_order_by_list = array(
            "id" => $partners_table . ".id",
            "company_name" => $partners_table . ".company_name",
            "created_date" => $partners_table . ".created_date",
        ...
        if ($search_by) {
            ...
            $where .= " AND (";
            $where .= " $partners_table.id LIKE '%$search_by%' ESCAPE '!' ";
            $where .= " OR $partners_table.company_name LIKE '%$search_by%' ESCAPE '!' ";
            $where .= " OR CONCAT($users_table.first_name, ' ', $users_table.last_name) LIKE '%$search_by%' ESCAPE '!' ";
            $where .= " OR (SELECT GROUP_CONCAT($labels_table.title, ', ') FROM $labels_table WHERE FIND_IN_SET($labels_table.id, $partners_table.labels)) LIKE '%$search_by%' ESCAPE '!' ";

            if ($leads_only) {
                $where .= " OR $lead_status_table.title LIKE '%$search_by%' ESCAPE '!' ";
                $where .= $this->get_custom_field_search_query($partners_table, "leads", $search_by);
            } else {
                $where .= $this->get_custom_field_search_query($partners_table, "clients", $search_by);
            }
