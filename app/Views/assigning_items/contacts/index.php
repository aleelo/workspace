    ...
    <div class="card rounded-bottom">
        <div class="tab-title clearfix">
            ...
            <div class="title-button-group">
                <?php
                if ($can_edit_clients) {
                    echo modal_anchor(get_uri("assigning_items/invitation_modal"), "<i data-feather='mail' class='icon-16'></i> " . app_lang('send_invitation'), array("class" => "btn btn-default", "title" => app_lang('send_invitation'), "data-post-partner_id" => $partner_id));

                    echo modal_anchor(get_uri("assigning_items/add_new_contact_modal_form"), "<i data-feather='plus-circle' class='icon-16'></i> " . app_lang('add_contact'), array("class" => "btn btn-default", "title" => app_lang('add_contact'), "data-post-partner_id" => $partner_id));
                }
                ?>
            </div>
        </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var quick_filters_dropdown = <?php echo view("assigning_items/contacts/quick_filters_dropdown"); ?>;
        ...
        $("#contact-table").appTable({
            source: '<?php echo_uri("assigning_items/contacts_list_data/" . $partner_id) ?>',
            serverSide: true,
