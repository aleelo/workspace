        ...
        <div class="row clients-view-button">
            <div class="col-md-12">
                <div class="page-title clearfix no-border no-border-top-radius no-bg">
                    <h1 class="pl0">
                        <?php echo app_lang('training_details') . " - " . $training_info->training_name ?>
                        <span id="star-mark">
                            <?php
                            if ($is_starred) {
                                echo view('training/star/starred', array("client_id" => $training_info->id));
                            } else {
                                echo view('training/star/not_starred', array("client_id" => $training_info->id));
                            }
                            ?>
                        </span>

                        <?php if ($training_info->lead_status_id) { ?>
                            <?php $lead_information = app_lang("past_lead_information") . "<br />"; ?>
                            <?php if ($training_info->created_date) { ?>
                                <?php $lead_information .= app_lang("lead_created_at") . ": " . format_to_date($training_info->created_date, false) . "<br />"; ?>
                            <?php } ?>
                            <?php if ($training_info->client_migration_date && is_date_exists($training_info->client_migration_date)) { ?>
                                <?php $lead_information .= app_lang("migrated_to_client_at") . ": " . format_to_date($training_info->client_migration_date, false) . "<br />"; ?>
                            <?php } ?>
                            <?php if ($training_info->last_lead_status) { ?>
                                <?php $lead_information .= app_lang("last_status") . ": " . $training_info->last_lead_status . "<br />"; ?>
                            <?php } ?>
                            <?php if ($training_info->owner_id) { ?>
                                <?php $lead_information .= app_lang("owner") . ": " . $training_info->owner_name; ?>
                            <?php } ?>
                    ...
                    <?php if (can_access_reminders_module()) { ?>
                        <div class="title-button-group mr0 clients-view">
                            <?php echo modal_anchor(get_uri("events/reminders"), "<i data-feather='clock' class='icon-16'></i> " . app_lang('reminders'), array("class" => "btn btn-default mr0", "id" => "reminder-icon", "data-post-client_id" => $training_info->id, "data-post-reminder_view_type" => "client", "title" => app_lang('reminders') . " (" . app_lang('private') . ")")); ?>
                        </div>
                    <?php } ?>
                </div>
                ...
                <ul id="client-tabs" data-bs-toggle="ajax-tab" class="nav nav-tabs scrollable-tabs" role="tablist">
                    ...
                    <li><a  role="presentation" data-bs-toggle="tab" href="<?php echo_uri("training/company_info_tab/" . $training_info->id); ?>" data-bs-target="#client-info"> <?php echo app_lang('training_info'); ?></a></li>



                    <?php
                    $hook_tabs = array();
                    $hook_tabs = app_hooks()->apply_filters('app_filter_client_details_ajax_tab', $hook_tabs, $training_info->id);
